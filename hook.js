...
// plain main-turn, a subagent's own kind, or the advisor. Local-storage-only, like
...
// `localStorage` gates the additional local.sqlite write.
...
function writeAggregatedEntries(sessionId, lines, startIdx = 0, { cwd = null, type = null } = {}) {
  // Lazy require + try/catch: local-storage.js is a separate pluginFiles entry with its
  // own auto-update lifecycle, so a mid-converge skew (this file landed, that one hasn't
  // yet) must never crash capture() itself — local storage is simply skipped, logged.
  let localStorage;
  try {
    localStorage = require('./local-storage.js');
  } catch (err) {
    logError('local-storage-module', err);
  }

  // Read once per call, not once per model — effectivePriceTable() re-reads prices.json.
  const priceTable = localStorage?.effectivePriceTable();
  const project = localStorage?.detectProject(cwd);
  const gitProject = localStorage?.detectGitProject(cwd);
  const branch = localStorage?.detectBranch(cwd);
  for (const [model, { usage, timestamp, ids, type: advisorType }] of aggregateUsage(lines, startIdx)) {
    ...
    localStorage?.writeLocalStorage({
      ...entry,
      price_cents: localStorage.priceCentsForModel(model, usage, priceTable),
      project,
