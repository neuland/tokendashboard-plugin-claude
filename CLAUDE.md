...
Four hooks registered in `~/.claude/settings.json` (command path is `$HOME/.claude/token-usage-plugin/hook.js` on POSIX):
...
`pct` appends ` COMPACT!` at 80%+. Line 2: `token-usage-plugin vVERSION · ↑in / ↓out tokens · $price` (network RX/TX convention: ↑ = sent/input tokens, ↓ = received/output tokens). 
...
| Path | Purpose |
|---|---|
| `~/.claude/token-usage-plugin/hook.js` | Installed hook script (auto-updated) |
| `~/.claude/token-usage-plugin/statusline.js` | Installed statusline script (auto-updated — a `pluginFiles` entry, same as `hook.js`) |
| `~/.claude/token-usage-plugin/config.json` | `currentVersion` + `lastUpdateCheck` timestamp + `migrationBlocked` flag + `apiBaseUrl`/`repoRawBaseUrl` (see ADR-016) |
| `~/.claude/token-usage-plugin/prices.json` | Cached price table (`schema`/`fetchedAt`/`table`), written by `hook.js`'s weekly `--fetch-prices`; read locally by `statusline.js` at render time — no network at render (see ADR-017) |
| `~/.claude/token-usage-plugin/user-id` | Random UUID for user pseudonymization |
| `~/.claude/token-usage-plugin/queue/` | Per-entry queue files (`[timestamp]-[pid].json`) |
| `~/.claude/token-usage-plugin/.update.lock` | Update lock (separate from the flush lock) |
...
- **Lock file** (`~/.claude/token-usage-plugin/queue/.lock`) must always be released in a `finally` block. Stale locks (owning process dead) are automatically stolen on next acquire.
...
- **`$HOME`-relative hook command.** The hook command is `$HOME/.claude/token-usage-plugin/hook.js` on POSIX so a mounted `~/.claude` works across host and devcontainer; it falls back to an absolute path on Windows or a custom `CLAUDE_CONFIG_DIR` outside `$HOME`. This string must be **identical** in both environments — `converge` uses it as the settings-drift key, so a divergence would ping-pong `settings.json`. `removeOwnHooks` matches on the substring `token-usage-plugin` (catches both the new and the legacy command), not a filename — do not narrow it back (see ADR-010, ADR-012).
