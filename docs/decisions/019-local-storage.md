# ADR-019: Local Storage Node Requirement and Worktree-Aware Attribution

## Decision

Local storage (`local.sqlite`) and `report.js` need `node:sqlite`, available from Node 22.13. `package.json`'s `engines.node` is `">=22.13"`.
...
- `install` checks the running Node and prints a note below the minimum, but never blocks or changes what gets installed — install/update copy the plugin's files
  ...
  which Node version was used to install or update. The note tells the user local storage/report need Node 22.13+ in whichever terminal Claude Code actually runs in — not to
  reinstall.
- Everything else (queue sync, statusline, price fetch) has no `node:sqlite` dependency and is unaffected by the Node version.
- `writeLocalStorage` logs `"Node 22.13+ needed for local storage (node:sqlite unavailable in <ver>)"` to `error.log` when `node:sqlite` is missing, instead of the raw
  `ERR_UNKNOWN_BUILTIN_MODULE` exception.
- `statusline.js` shows a red `⚠ local storage off (Node <ver> < 22.13)` segment only when local storage has previously produced data (`local.sqlite` exists) AND `node:sqlite`
  is currently unavailable. An install that has never produced local-storage data stays silent.
...
- Local storage/report are additive; failing the whole install over one optional feature would take away the queue/backend telemetry path too.
...
- The statusline only warns on a below-minimum Node when `local.sqlite` already exists: that user relies on local storage and expects the current session's tokens to be
...
- An isolated `git worktree` (e.g. an agent-per-worktree workflow) has its own transient folder name and branch, meaningless for reporting; resolving through the original
  worktree attributes usage to the project the agent actually started from. For a plain, non-worktree repo, resolution is a no-op.
...
- **An install-time `config.json` flag for "local storage should be active"**: rejected — would never retroactively appear for already-installed users, since auto-update never
  re-runs `install()`.
